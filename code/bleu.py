...
BEG_token = 1
EOS_token = 2
with open('../data/ENGindex2word.pkl', 'rb') as f:
    ENGindex2word = pickle.load(f, encoding='bytes')
with open('../data/CHIindex2word.pkl', 'rb') as f:
    CHIindex2word = pickle.load(f, encoding='bytes')

def getlist(path, lang):
    # lang = 0 ->eng
    # lang = 1 ->chi
    ...
    for i, sentence in enumerate(sentences):
        ...
        if lang == 0:
            temp = [ENGindex2word[i] for i in temp]
        else:
            temp = [CHIindex2word[i] for i in temp]
        sentences[i] = temp
    return sentences

if __name__ == '__main__':
    src = getlist('../data/test/test_src.pkl', 0)
    tgt = getlist('../data/test/test_tgt.pkl', 1)
    gen = getlist('../data/test/test_gen.pkl', 1)
    ...
    print('BLEU is {:2f}'.format(bleu / len(gen))) #41.5184%
